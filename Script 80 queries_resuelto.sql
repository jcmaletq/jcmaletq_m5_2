departamentosdepartamentoscreate database queries_mysql;
use queries_mysql;
...
use queries_mysql;
select emp.EMPNO, ENAME,JOB, MGR,date_format(HIREDATE,'%d/%m/%y'),SAL,COMM, DEPTNO from empleados emp;
...
use queries_mysql;
select dep.DEPTNO,DNAME,LOC from departamentos dep;
...
use queries_mysql;
select emp.EMPNO, ENAME,SAL from empleados emp;
...
use queries_mysql;
select ENAME,JOB from empleados emp
order by sal;
...
select sal,comm from empleados emp
where deptno = '30';
...
select comm from empleados emp
;
...
select distinct comm from empleados emp
...
select distinct comm comission, deptno from empleados emp
order by deptno
;
...
select ename, sal+1000 'sal+1000' from empleados emp
where deptno = '30'
;
...
6 filas seleccionadas.*/
select ename, sal,  sal+1000 'nuevo salario' from empleados emp
where deptno = '30'
;
...
select ename from empleados emp
where comm > sal/2
;
...
select concat ('Nombre: ',ename) as nombre, concat ('Salario: ',sal) from empleados emp
;
...
select empno, sal, comm from empleados emp
where empno > 7500;
...
from empleados
where comm is null;
/*  considero que es mejor la de abajo*/
select sal, comm , sal+ifnull(comm,0) as salario_total
from empleados
where comm is null;
...
14 filas seleccionadas.*/
select comm/(sal+comm)*100 as porcentaje
from empleados
order by ename;
/* abajo trata los nulos */
select ifnull(comm,0)/(sal+ifnull(comm,0))*100 as porcentaje
from empleados
order by ename;
...
from empleados emp
/* inner join empleados adm on adm.empno = emp.mgr*/
where emp.deptno <> 10
and emp.job = 'CLERK'
and emp.sal > 800
...
select emp.ename, emp.sal/comm as cociente
from empleados emp
where emp.comm is not null and comm > 0
order by ename
;
...
select emp.EMPNO, ENAME,JOB, MGR,date_format(HIREDATE,'%d/%m/%y') as hiredate ,SAL,COMM, DEPTNO 
from empleados emp
...
and sal > 1000)
or (comm is not null and comm > 0
and deptno = 30)
;
/* tambíen se puede hacer con unions*/
...
and sal > 1000
union
select emp.EMPNO, ENAME,JOB, MGR,date_format(HIREDATE,'%d/%m/%y'),SAL,COMM, DEPTNO 
from empleados emp
...
select distinct emp.ename, emp.job
from empleados emp
WHERE  CONCAT(ENAME, JOB) REGEXP ('^A.*MAN$');

/* se pueden usar estas tambíen*/
select distinct emp.ename, emp.job
from empleados emp
inner join  empleados em on emp.empno=em.empno and left(em.ename,1) = 'A'
where emp.job like '%MAN';
/* o bien usando subqueris*/
select distinct t1.ename, t1.job
from (select distinct emp.ename, emp.job from empleados emp where emp.job like '%MAN') t1
where left(t1.ename,1) = 'A';
...
format(((sal+ifnull(comm,0))+(sal+ifnull(comm,0))*60/100),0) as salario_proximo ,  
format((((sal+ifnull(comm,0))+(sal+ifnull(comm,0))*60/100) +((sal+ifnull(comm,0))+(sal+ifnull(comm,0))*60/100)*70/100),0) as salario_siguiente
from empleados emp
order by ename
;
...
select empno, ENAME, job, mgr, date_format(HIREDATE,'%d/%m/%y') as hiredate, sal, comm, deptno
 from empleados emp
...
select max(sal) as max_sal, sum(comm) as sum_com, count(*)
from empleados
;
...
select emp.empno, emp.ENAME, emp.job, emp.mgr, date_format(emp.HIREDATE,'%d/%m/%y') as hiredate, emp.sal, emp.comm, emp.deptno
 from empleados emp
where sal > (select distinct t.sal from empleados t where t.empno = 7934) 
order by emp.sal;
...
from empleados emp
where sal >= (select distinct t.sal from empleados t where t.ename = 'ALLEN') 
;
...
from empleados e
where e.sal = (select max(sal)from empleados emp order by ename)
or e.sal = (select min(sal) from empleados emp order by ename)
order by sal desc
;
...
from empleados e
inner join departamentos d on d.deptno = e.deptno
where sal < 5000
and d.deptno in (select distinct deptno from empleados group by deptno having min(sal) >900)
group by e.deptno
;
...
select a.ename
from empleados a
where a.sal >= (select avg(e.sal) from empleados e)
;
...
select a.ename
from empleados a
inner join  (select distinct e.deptno as dep , max(e.sal) as ma from empleados e group by e.deptno) f on f.dep = a.deptno and a.sal = f.ma 
;
...
select count( distinct job) as num_job,  count(distinct empno) as num_emp,  count(distinct sal) as num_sal, sum(sal) as sala
from empleados
where deptno = 30
order by job, empno, sal
;
...
select count( distinct comm) as num_comm
from empleados
where comm > 0;
...
select count( distinct empno) as num_emp
from empleados
where deptno = 20;
...
select deptno, count(distinct empno) as num_empl
from empleados
group by deptno
...
from empleados e
where e.deptno = 10
and e.job in (select distinct a.job from empleados a inner join departamentos d on a.deptno = d.deptno and d.dname ='SALES')
;
...
select distinct m.ename /*, count(distinct e.empno) */
from empleados e
inner join empleados m on m.empno = e.mgr
...
select emp.empno, emp.ENAME, emp.job, emp.mgr, date_format(emp.HIREDATE,'%d/%m/%y') as hiredate, emp.sal, emp.comm, emp.deptno
 from empleados emp
where job in (select distinct t.job from empleados t inner join departamentos d on d.deptno = t.deptno where d.loc = 'CHICAGO') 
...
select distinct job, count(distinct empno) as count
from empleados
group by job
;
...
select distinct d.dname, sum(e.sal)
from departamentos d
inner join empleados e on e.deptno = d.deptno
group by d.dname
;
...
from departamentos d
left outer join empleados e on e.deptno = d.deptno
...
from empleados e
left outer join empleados s on e.empno = s.mgr
where s.mgr is  null 
;

/* pero me parece mejor esta*/
select distinct e.ename 
from empleados e
left outer join empleados s on e.empno = s.mgr
where s.mgr is  null 
...
select d.dname, avg (e.sal) as medi_sal, count(distinct e.empno)
from departamentos d
inner join empleados e on d.deptno = e.deptno
group by d.dname;
...
where deptno = 30
order by comm desc;
/* el orden no es el mismo por los nulos*/
...
select e.ename as empleado, m.ename as manager
from empleados e
left outer join empleados m on m.empno = e.mgr
order by m.ename
;
...
from empleados a
inner join  (select distinct e.deptno as dep , max(e.sal) as ma from empleados e group by e.deptno) f on f.dep = a.deptno and a.sal = f.ma 
inner join departamentos d on d.deptno = a.deptno
order by a.sal
;
...
select  m.ename, m.empno , count(distinct e.empno) as num_emp
from empleados e
left outer join empleados m on m.empno = e.mgr
group by m.ename
order by m.ename
...
use queries_mysql;
select d.dname, f.ma as sum_salar
from departamentos d
inner join  (select distinct t.* from (select  e.deptno as dep , sum(e.sal) as ma from empleados e  group by e.deptno  order by sum(e.sal) desc limit 1) t ) f 
on f.dep = d.deptno 
;

/* esta otra no usa limit */

select d.dname, f.ma as sum_salar
from departamentos d
inner join  (select @rownum:=@rownum+1 as nr, t.dep, t.ma from (select  e.deptno as dep , sum(e.sal) as ma from empleados e  group by e.deptno  order by sum(e.sal) desc) t , (SELECT @rownum:=0) r) f 
on f.dep = d.deptno and f.nr = 1
;
...
select * 
from (
select  empno, ename, job, mgr,  date_format(e.HIREDATE,'%d/%m/%y') as hiredate, sal, comm, deptno
from empleados e
order by sal desc) t
limit 3;
/* esta otra no usa limit*/
select * 
from (
select @rownum:=@rownum+1 as nr, empno, ename, job, mgr,  date_format(e.HIREDATE,'%d/%m/%y') as hiredate, sal, comm, deptno
from empleados e, (SELECT @rownum:=0) r
order by sal desc) t
where t.nr <=3;
...
select loc,  count(distinct e.empno) as numero_empleados
from departamentos d
inner join empleados e on e.deptno = d.deptno
group by loc, dname
having count(distinct e.empno) >= 4
